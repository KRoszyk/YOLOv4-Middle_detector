...
def cspdarknet53(input_data):
    input_data = convolutional(input_data, (3, 3, 4, 32), activate_type="mish")
...
def YOLOv4_tiny_modified(input_layer, NUM_CLASS):
    route_1_1, conv_1 = cspdarknet53_tiny(input_layer[:, :, :, 0:3])
    route_1_2, conv_2 = cspdarknet53_tiny(input_layer[:, :, :, 3:6])

    route_1 = tf.concat([route_1_1, route_1_2], axis=-1)
    conv = tf.concat([conv_1, conv_2], axis=-1)
...
def YOLOv4_modified(input_layer, NUM_CLASS):
    # Modified YOLO v4 - 2 backbones
    input_rgb = input_layer[:, :, :, 0:3]
    input_thermo = input_layer[:, :, :, 3:6]
    route_1_1, route_2_1, conv_1 = cspdarknet53(input_rgb)
    route_1_2, route_2_2, conv_2 = cspdarknet53(input_thermo)

    route_1 = tf.concat([route_1_1, route_1_2], axis=-1)
    route_2 = tf.concat([route_2_1, route_2_2], axis=-1)
    conv = tf.concat([conv_1, conv_2], axis=-1)

    route_1 = convolutional(route_1, (1, 1, 512, 256))
    route_2 = convolutional(route_2, (1, 1, 1024, 512))
    conv = convolutional(conv, (1, 1, 1024, 512))
...
def Create_Yolo(input_size=416, channels=6, training=False, CLASSES=YOLO_COCO_CLASSES, modified=False):
    NUM_CLASS = len(read_class_names(CLASSES))
    input_layer = Input([input_size, input_size, channels])

    if TRAIN_YOLO_TINY:
        if modified:
            conv_tensors = YOLOv4_tiny_modified(input_layer, NUM_CLASS)
        else:
            conv_tensors = YOLOv4_tiny(input_layer, NUM_CLASS)
    else:
        if modified:
            conv_tensors = YOLOv4_modified(input_layer, NUM_CLASS)
        else:
