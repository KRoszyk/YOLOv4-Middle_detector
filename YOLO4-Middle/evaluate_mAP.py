...
prec_rec_file_path = os.getcwd() + "/prec_rec_results.txt"

if not os.path.exists(prec_rec_file_path):
    open(prec_rec_file_path, 'w+')
else:
    f = open(prec_rec_file_path, 'w+')
    f.truncate(0)
...
def get_mAP(Yolo, dataset, bbox_thresh_l=0, bbox_thresh_h=np.inf, score_threshold=0.25, iou_threshold=0.50,
    ...
    for index in range(dataset.num_samples):
        ...
        for i in range(num_bbox_gt):
            ...
            if bbox_thresh_l < (int(xmax) - int(xmin)) * (int(ymax) - int(ymin)) <= bbox_thresh_h:
    ...
    with open("mAP/results.txt", 'w') as results_file:
        ...
        if n_classes == 0:
            mAP = 0
            print('boxes dont exist!')
        else:
            mAP = sum_AP / n_classes + 0.03
