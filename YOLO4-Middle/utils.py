...
from configs import *
from yolov4_f import *
from tensorflow.python.saved_model import tag_constants
import os


def create_whl_annotations(whl_file, annot_path, bboxes):
    wormhole_file = open(whl_file, "a")
    output_line = annot_path + " "

    for i, bbox in enumerate(bboxes):
        coor = np.array(bbox[:4], dtype=np.int32)
        (x1, y1), (x2, y2) = (coor[0], coor[1]), (coor[2], coor[3])
        output_line += str(x1) + "," + str(y1) + "," + str(x2) + "," + str(y2) + "," + "0" + " "

    output_line += "\n"
    wormhole_file.write(output_line)
    wormhole_file.close()
...
def image_preprocess(image, target_size, gt_boxes=None):
    ...
    image_paded = np.full(shape=[ih, iw, 6], fill_value=128.0)
    dw, dh = (iw - nw) // 2, (ih - nh) // 2
    dw, dh = 0, 0
    image_paded[dh:nh + dh, dw:nw + dw, :] = image
...
def detect_image(Yolo, image_path, output_path, input_size=416, show=False, CLASSES=YOLO_COCO_CLASSES,
                 score_threshold=0.52, iou_threshold=0.45, rectangle_colors='', draw_bboxes=True):
